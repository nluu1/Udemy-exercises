...
#merge(left data, right data, by.x = which column to use from left data, by.y = which column from right data)
merged.df <- merge(stats,mydf,
                   by.x = "Country.Code", 
                   by.y = "Code")
View(merged.df)
...
qplot(data=merged.df, x=Internet.users, y= Birth.rate)

qplot(data=merged.df, x=Internet.users, y= Birth.rate,
      color=Region) #add color by region

#Add Shape (look up R shape number code from google)
qplot(data=merged.df, x=Internet.users, y= Birth.rate,
